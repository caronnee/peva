...
[A-Za-z]+[A-Z_a-z0-9]* 				{*l = line;lv->ident.id = yytext; return TOKEN_IDENTIFIER; };
[0-9]+		 	 	 	 	{*l = line;lv->ident.id = yytext; lv->number = atoi(yytext);return TOKEN_UINT; };
[0-9]+[A-Za-z0-9]* 	 	 	 	{*l = line;lv->ident.id = yytext; lv->number = atoi(yytext);return TOKEN_UINT; };
[0-9]+(\.[0-9]+)?([Ee][\+\-]?[0-9]+)?		{*l = line;lv->ident.id = yytext; lv->f_number = atof(yytext);return TOKEN_REAL; };
