...
names:	TOKEN_IDENTIFIER { $$.push_back(Constr($1)); } //nic loaded
	|TOKEN_IDENTIFIER TOKEN_ASSIGN expression { $$.push_back(Constr($1,$3.ins, $3.output)); std::cout << "pocit instrukcii:" << $3.ins.size(); } 
     	|TOKEN_IDENTIFIER TOKEN_ASSIGN TOKEN_BEGIN values TOKEN_END { $$.push_back(Constr($1, $4.ins, $4.output));std::cout << "pocet naloadovanych:" << $4.output.size();getc(stdin); } 
 ...
 $1.push_back(Constr($3, $6.ins, $6.output));$$ = $1; } //inizializcia array a Location, laod vsetkych premenntch a poto spotupne storovanie
	;
...
values: expression { $$ = $1; } // blabla load nieco, {Instruction, Type}
	| values TOKEN_COMMA expression {$$ = $1; $$.ins.push_back(NULL);$$.ins = join_instructions($$.ins, $3.ins); $$.output.push_back($3.output[0]);}
